...
export class MockDataService {
  ...
  private generateData(): TableData {
    ...
    return new Array(this.targetDataLength).fill(undefined).map<TableDataItem>(() => {
  ...
  private generatePlatforms(): Array<TableDataItem['platform']> {
    return new Array(this.targetPlatformsLength).fill(undefined).map(() => this.capitalize(internet.domainWord()));
  }
